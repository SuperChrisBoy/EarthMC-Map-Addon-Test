...
public final class XaeroRadiusOverlayRenderer {
    ...
    private static void ring(GuiGraphicsExtractor ctx,double cx,double cy,double radius,int color,int thickness){if(radius<2)return;int segments=Math.clamp((int)Math.ceil(radius*.75),24,128);double px=cx+radius,py=cy;for(int i=1;i<=segments;i++){double a=Math.PI*2*i/segments,nx=cx+Math.cos(a)*radius,ny=cy+Math.sin(a)*radius;segment(ctx,px,py,nx,ny,color,thickness);px=nx;py=ny;}}
    private static void segment(GuiGraphicsExtractor ctx,double x1,double y1,double x2,double y2,int color,int thickness){double dx=x2-x1,dy=y2-y1;int length=(int)Math.ceil(Math.hypot(dx,dy));if(length<=0)return;var m=ctx.pose();m.pushMatrix();try{m.translate((float)x1,(float)y1);m.rotate((float)Math.atan2(dy,dx));ctx.fill(0,-thickness/2,length+1,(thickness+1)/2,color);}finally{m.popMatrix();}}
}
