...
public final class HunterActivityOverlay {
    ...
    public static void render(DrawContext c,int sw,int sh,TownyMapConfig cfg,boolean interactive){
        if(!cfg.hunterActivityWindowShown){cancelDrag();return;}
        MinecraftClient mc=MinecraftClient.getInstance();
        if(dragging&&(!interactive||mc==null||!mc.isWindowFocused()||org.lwjgl.glfw.GLFW.glfwGetMouseButton(mc.getWindow().getHandle(),org.lwjgl.glfw.GLFW.GLFW_MOUSE_BUTTON_LEFT)!=org.lwjgl.glfw.GLFW.GLFW_PRESS)){cancelDrag();}
        if(dragging){double mx=mc.mouse.getX()*sw/mc.getWindow().getWidth(),my=mc.mouse.getY()*sh/mc.getWindow().getHeight();cfg.hunterActivityWindowX=HunterActivityWindowState.draggedX(mx,dragOffsetX,sw,W);cfg.hunterActivityWindowY=HunterActivityWindowState.draggedY(my,dragOffsetY,sh,HEADER);}
        ...
        c.drawText(mc.textRenderer,Text.translatable("townymapaddon.hunter.activity.title"),x+PAD,y+7,0xFFFFFFFF,false);
        c.drawText(mc.textRenderer,cfg.hunterActivityWindowMinimized?"[+] [X]":"[–] [X]",x+W-46,y+7,0xFFBFC5CC,false);
        ...
        for(HunterEvent e:events){if(skipped++<scroll)continue;int color=switch(e.severity()){case CRITICAL->0xFFFF5555;case WARNING->0xFFFFAA55;case NOTICE->0xFFFFFF55;case INFO->0xFFBFC5CC;};String head=TIME.format(Instant.ofEpochMilli(e.atMs()))+"  "+e.title().getString();c.drawText(mc.textRenderer,trim(head,35),x+PAD,cy,color,false);cy+=LINE;for(var line:e.lines()){if(cy+LINE>y+h-3)break;c.drawText(mc.textRenderer,trim(line.getString(),38),x+PAD+8,cy,0xFF9FA6AE,false);cy+=LINE;}cy+=4;if(cy+LINE>y+h)break;}
    }
