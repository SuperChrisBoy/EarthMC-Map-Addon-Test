...
public class DetailScreen extends Screen {
    ...
    private ButtonWidget backButton, discordButton, wikiButton;
    ...
    public DetailScreen(Screen parent, String pendingTitle) {
        super(Text.literal(pendingTitle));
        this.parent = parent;
        this.page = null;
    ...
    public void setPage(Page p) {
        this.page = p;
        this.failed = false;
        if (this.client != null) this.clearAndInit();
    }
    ...
    protected void init() {
        ...
        if (page != null && page.discordUrl() != null && !page.discordUrl().isBlank()) {
            discordButton = ButtonWidget.builder(Text.literal("Discord"),
                            x -> TownInfoOverlay.openDiscord(page.discordUrl()))
                    .dimensions(bx, footerButtonY(), 64, 20).build();
            discordButton.setTooltip(Tooltip.of(Text.literal(page.discordUrl())));
            this.addDrawableChild(discordButton);
            bx += 64 + GAP;
        }
        if (page != null && page.wikiUrl() != null && !page.wikiUrl().isBlank()) {
            wikiButton = ButtonWidget.builder(Text.literal("Wiki"),
                            x -> TownInfoOverlay.openDiscord(page.wikiUrl()))
                    .dimensions(bx, footerButtonY(), 52, 20).build();
            this.addDrawableChild(wikiButton);
        }
        backButton = ButtonWidget.builder(
                        Text.literal(parent == null ? "Close" : "Back"), x -> this.close())
                .dimensions(cRight - 64, footerButtonY(), 64, 20).build();
    ...
    private void renderSearchResults(DrawContext ctx, int mouseX, int mouseY) {
        ...
        if (hasSearch()) {
            ...
            ctx.drawText(this.textRenderer, this.textRenderer.trimToWidth(shown, cRight - cLeft - 12),
                    cLeft + 6, fy + 5, empty ? 0xFF7A7A7A : 0xFFFFFFFF, false);
            if (searchFocused && (System.currentTimeMillis() / 500) % 2 == 0) {
                int cx = cLeft + 6 + this.textRenderer.getWidth(searchQuery);
        ...
        for (int i = 0; i < searchHits.size(); i++) {
            ...
            ctx.drawText(this.textRenderer, ref.name(), cLeft + 4, ry + 2, hot ? HOVER : LINK, false);
            String kindLabel = ref.kind().name().charAt(0) + ref.kind().name().substring(1).toLowerCase(java.util.Locale.ROOT);
            ctx.drawText(this.textRenderer, kindLabel, cRight - this.textRenderer.getWidth(kindLabel) - 4, ry + 2, MUTED, false);
    ...
    public boolean charTyped(CharInput input) {
        if (!searchFocused || !hasSearch() || !input.isValidChar()) {
    ...
    private void drawContent(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        if (sub != null && !sub.isBlank()) {
            ctx.drawText(this.textRenderer, sub,
                    cLeft + this.textRenderer.getWidth(title) + 6, PANEL_TOP + 9, MUTED, false);
        }
        ...
        for (int i = 0; i < TABS.length; i++) {
            int w = this.textRenderer.getWidth(TABS[i]) + 12;
            ...
            ctx.drawText(this.textRenderer, TABS[i], tx + 6, ty + 4, active ? 0xFFFFFFFF : MUTED, false);
        ...
        if (hasSubTabs()) {
            ...
            for (int i = 0; i < SUBTABS.length; i++) {
                int w = this.textRenderer.getWidth(SUBTABS[i]) + 10;
                ...
                ctx.drawText(this.textRenderer, SUBTABS[i], sx + 5, sy + 4, act ? 0xFFFFFFFF : MUTED, false);
        ...
        if (hasSubTabs()) {
            ...
            for (int i = 0; i < filters.length && i < filterRects.length; i++) {
                int w = this.textRenderer.getWidth(filters[i]) + 10;
                ...
                ctx.drawText(this.textRenderer, filters[i], fx + 5, fy + 4, act ? 0xFFFFFFFF : MUTED, false);
        ...
        if (page == null) {
            ...
            ctx.drawText(this.textRenderer, msg, cLeft, bodyTop() + 10, MUTED, false);
            contentHeight = 30;
        } else {
        ...
        if (DarkButtons.enabled()) {
            for (var child : this.children()) {
                if (child instanceof net.minecraft.client.gui.widget.ClickableWidget w && w.visible) {
    ...
    private int drawBlocks(DrawContext ctx, int mouseX, int mouseY) {
        ...
        for (Block b : page.blocks()) {
            if (b instanceof Rule) {
                ...
                ctx.drawText(this.textRenderer, w.label(), cLeft, y, LABEL, false);
                y += LINE;
                for (var l : this.textRenderer.wrapLines(Text.literal(w.value()), cRight - cLeft)) {
                    ctx.drawText(this.textRenderer, l, cLeft, y, VALUE, false);
                    y += LINE;
                }
                y += 3;
                ...
                for (int i = 0; i < c.cols().size(); i++) {
                    ...
                    ctx.drawText(this.textRenderer, col.label(), x, y, LABEL, false);
                    boolean link = col.link() != null;
                    String shown = this.textRenderer.trimToWidth(col.text(), colW);
                    int w = this.textRenderer.getWidth(shown);
                    boolean hot = link && mouseX >= x && mouseX < x + w
                            && mouseY >= y + LINE - 1 && mouseY < y + LINE + 9
                            && mouseY >= bodyTop() && mouseY < bodyBottom();
                    ctx.drawText(this.textRenderer, shown, x, y + LINE,
                ...
                for (int i = 0; i < ch.labels().size(); i++) {
                    ...
                    int w = this.textRenderer.getWidth(lab) + 10;
                    if (x + w > cRight) { x = cLeft; y += 13; }
                    ctx.fill(x, y - 1, x + w, y + 10, on ? 0x557FE0B0 : 0x1AFFFFFF);
                    ctx.drawText(this.textRenderer, lab, x + 5, y + 1, on ? ON_FG : OFF_FG, false);
                    x += w + 4;
                }
                y += 17;
    ...
    private int drawCollapsible(DrawContext ctx, int mouseX, int mouseY, int y,
        ...
        if (!bare) {
            ...
            ctx.drawText(this.textRenderer, (isOpen ? "\u25be " : "\u25b8 ") + title, cLeft + 6, y,
                    isOpen ? 0xFFFFFFFF : VALUE, false);
            String c = String.valueOf(count);
            ctx.drawText(this.textRenderer, c, cRight - this.textRenderer.getWidth(c), y, LABEL, false);
        ...
        if (body instanceof NameList nl) {
            ...
            for (int i = 0; i < nl.refs().size(); i++) {
                ...
                boolean hot = mouseX >= x && mouseX < x + w && mouseY >= y - 1 && mouseY < y + 9
                        && mouseY >= bodyTop() && mouseY < bodyBottom();
                ctx.drawText(this.textRenderer, txt, x, y, hot ? HOVER : LINK, false);
            ...
            for (RankGroup g : rl.groups()) {
                int x = cLeft + 10;
                String head = g.rank() + ": ";
                ctx.drawText(this.textRenderer, head, x, y, LABEL, false);
                x += this.textRenderer.getWidth(head);
            ...
            for (RefLine rl : rll.lines()) {
                ...
                if (rll.ranked()) {
                    ...
                    String num = rank + ".";
                    ctx.drawText(this.textRenderer, num, cLeft + 12, y, MUTED, false);
                    int nx = cLeft + 12 + 22;
                    int nw = this.textRenderer.getWidth(nm);
                    ctx.drawText(this.textRenderer, nm, nx, y, rowHot ? HOVER : LINK, false);
                    ...
                    if (val != null && !val.isBlank()) {
                        ...
                        ctx.drawText(this.textRenderer, this.textRenderer.trimToWidth(val, cRight - 8 - vx),
                                vx, y, MUTED, false);
                    }
                } else {
                    int x = cLeft + 10;
                    int w = this.textRenderer.getWidth(nm);
                    boolean hot = mouseX >= x && mouseX < x + w && mouseY >= y - 1 && mouseY < y + 9
                            && mouseY >= bodyTop() && mouseY < bodyBottom();
                    ctx.drawText(this.textRenderer, nm, x, y, hot ? HOVER : LINK, false);
                    hits.add(new Hit(x, y - 1, x + w, y + 9, rl.ref(), null));
                    ctx.drawText(this.textRenderer,
                            this.textRenderer.trimToWidth(rl.suffix(), cRight - (x + w + 5)),
                            x + w + 5, y, MUTED, false);
                }
                y += LINE;
            }
            ...
            for (String line : ll.lines()) {
                for (var l : this.textRenderer.wrapLines(Text.literal(line), cRight - cLeft - 10)) {
                    ctx.drawText(this.textRenderer, l, cLeft + 10, y, MUTED, false);
                    y += LINE;
                }
            }
            y += 3;
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (mx < panelLeft || mx > panelLeft + panelWidth || my < PANEL_TOP || my > panelBottom) {
            this.client.setScreen(rootParent());
            return true;
        }
