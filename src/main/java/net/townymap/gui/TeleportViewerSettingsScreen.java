...
public final class TeleportViewerSettingsScreen extends Screen {
    ...
    @Override protected void init(){cfg=TownyMapMod.getConfig();int left=Math.max(12,(width-560)/2),x=left+270,y=58;
        ...
        if(!TownyMapMod.isOnEarthMcServer())addRenderableWidget(Button.builder(Component.translatable(cfg.teleportAllowNonEarthMc?"options.on":"options.off"),b->{cfg.teleportAllowNonEarthMc=!cfg.teleportAllowNonEarthMc;cfg.save();rebuildWidgets();}).bounds(x,350,280,20).build());
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float d){int left=Math.max(12,(width-560)/2),right=Math.min(width-12,left+560);g.fill(left-12,16,right+12,height-8,0xF2080C10);g.fill(left-9,19,right+9,height-11,0xFA10171D);super.extractRenderState(g,mx,my,d);g.centeredText(font,title,width/2,30,0xFFFFFFFF);g.text(font,Component.translatable("townymapaddon.teleport.primary_town"),left+18,64,0xFFE4E9ED,false);String[] keys={"townymapaddon.teleport.settings.advanced_enabled","townymapaddon.teleport.settings.command_action","townymapaddon.teleport.settings.town_spawns","townymapaddon.teleport.settings.nation_spawns","townymapaddon.teleport.settings.route_line","townymapaddon.teleport.settings.target_marker","townymapaddon.teleport.settings.arrival_marker"};int py=102;for(String key:keys){g.text(font,Component.translatable(key),left+18,py,0xFFE0E6EA,false);if(key.endsWith("command_action")){if(cfg.teleportCommandAction==2)g.text(font,Component.translatable("townymapaddon.settings.execute_command_warning"),left+288,py+20,0xFFFF5555,false);py+=40;}else py+=27;}g.text(font,Component.translatable("townymapaddon.teleport.settings.hint"),left+18,py+12,0xFFAFBAC2,false);if(!TownyMapMod.isOnEarthMcServer()){g.text(font,Component.translatable("townymapaddon.safety.non_earthmc_warning"),left+18,332,0xFFFF5555,false);g.text(font,Component.translatable("townymapaddon.safety.teleport_override"),left+18,356,0xFFFF7777,false);}if(!status.isBlank())g.text(font,status,left+18,height-48,0xFFFFCC66,false);}
