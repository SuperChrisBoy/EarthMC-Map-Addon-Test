...
public final class TeleportViewerOverlay {
    ...
    public static void open(double x,double z){targetX=x;targetZ=z;open=true;minimized=false;TownyMapConfig cfg=TownyMapMod.getConfig();advanced=cfg.teleportAdvancedEnabled&&cfg.teleportDefaultAdvanced;selected=scroll=0;resultsCleared=false;advancedConfirmed=false;feedback="";currentPlan=null;currentRoutes=List.of();TownyMapMod.refreshTeleportData(x,z);}
    ...
    public static void render(GuiGraphicsExtractor g,double camX,double camZ,double mapScale,int sw,int sh,TownyMapConfig cfg){
        ...
        int topOffset=advanced?106:94;int h=minimized?HEADER:Math.min(sh-y-6,topOffset+VISIBLE*ROW+53);
        ...
        drawRefreshButton(g,x+W-78,y+29,68,19,plan!=null&&plan.loading());
    ...
    public static boolean click(double mx,double my,int sw,int sh,TownyMapConfig cfg){
        if(!open)return false;int x=Math.clamp(cfg.teleportWindowX,4,Math.max(4,sw-W-4)),y=Math.clamp(cfg.teleportWindowY,4,Math.max(4,sh-HEADER-4));int topOffset=advanced?106:94;int h=Math.min(sh-y-6,topOffset+VISIBLE*ROW+53);
        ...
        if(mx>=x+W-78&&mx<x+W-10&&my>=y+29&&my<y+48){if(currentPlan==null||!currentPlan.loading()){resultsCleared=false;currentPlan=null;currentRoutes=List.of();selected=scroll=0;TownyMapMod.forceRefreshTeleportData(targetX,targetZ);}return true;}
