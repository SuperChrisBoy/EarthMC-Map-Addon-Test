...
public final class TeleportViewerOverlay {
    ...
    public static void open(double x,double z){targetX=x;targetZ=z;open=true;minimized=false;TownyMapConfig cfg=TownyMapMod.getConfig();advanced=cfg.teleportAdvancedEnabled&&cfg.teleportDefaultAdvanced;selected=scroll=0;resultsCleared=false;advancedConfirmed=false;feedback="";currentPlan=null;currentRoutes=List.of();TownyMapMod.refreshTeleportData();}
    ...
    public static void render(GuiGraphicsExtractor g,double camX,double camZ,double mapScale,int sw,int sh,TownyMapConfig cfg){
        ...
        int topOffset=advanced?106:94,h=minimized?HEADER:Math.min(sh-y-6,topOffset+VISIBLE*ROW+53);
    ...
    public static boolean click(double mx,double my,int sw,int sh,TownyMapConfig cfg){
        if(!open)return false;int x=Math.clamp(cfg.teleportWindowX,4,Math.max(4,sw-W-4)),y=Math.clamp(cfg.teleportWindowY,4,Math.max(4,sh-HEADER-4));int topOffset=advanced?106:94,h=Math.min(sh-y-6,topOffset+VISIBLE*ROW+53);
        ...
        int top=y+topOffset,index=scroll+(int)((my-top)/ROW);if(index>=scroll&&index<currentRoutes.size()&&index<scroll+VISIBLE){TeleportRoute route=currentRoutes.get(index);int rowY=top+(index-scroll)*ROW;boolean insideCommandX=mx>=x+14&&mx<x+W-14;if(!advancedLocked()&&insideCommandX){if(advanced&&my>=rowY+17&&my<rowY+31){String join=stepCommand(route,TeleportRoute.StepType.JOIN_TOWN);if(!join.isBlank())perform(join,route);return true;}if(advanced&&my>=rowY+34&&my<rowY+48){perform(route.destination().command(),route);return true;}if(!advanced&&my>=rowY+20&&my<rowY+37){perform(route.destination().command(),route);return true;}}selected=index;return true;}return true;
    }
    private static void perform(String command,TeleportRoute route){TeleportCommandAction.Result result=TeleportCommandAction.perform(command,TownyMapMod.getConfig().teleportCommandAction,advanced,route.destination().eligibility()==TeleportDestination.Eligibility.ACCESSIBLE);feedback=Component.translatable("townymapaddon.teleport.command_result."+result.name().toLowerCase(Locale.ROOT)).getString();feedbackUntil=System.currentTimeMillis()+1_600;}
