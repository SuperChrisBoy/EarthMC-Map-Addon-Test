...
public final class HunterEarlyWarningSystem {
    ...
    public void tick(MinecraftClient mc, List<PlayerMarker> players, List<TownData> townSnapshot) {
        towns=townSnapshot;
        candidates.tick(townSnapshot);
        long now=System.currentTimeMillis();
        if (!config.hunterWarningEnabled || mc.player == null || mc.getSession() == null){if(mc.player==null){exposureSession.reset();playerIdentity=null;}threatFronts.clear();radiusOverlays.clear();minimapRadiusOverlays.clear();logHealth(now);return;}
        ...
        updateNearbyCandidates(players,px,pz,scale,now,mc.getSession().getUsername());
        TownData userTown=TownHoverOverlay.townAt(px,pz,towns); boolean selfVisible=map.containsKey(key(mc.getSession().getUsername())); boolean unsafe=userTown==null||Boolean.TRUE.equals(ruinStatus.get(key(userTown.name()))); exposure.update(selfVisible,unsafe,now);
    ...
    private void refreshApproaches(MinecraftClient mc,long now,double px,double pz){
    ...
    private void publishRouteChange(HunterState h,List<net.townymap.hunter.model.ApproachRoute> before,List<net.townymap.hunter.model.ApproachRoute> after,long now){
        ...
        if(!fresh.equals(old)){String summary=after.isEmpty()?"No verified nearby spawn access":after.stream().map(r->r.name()+" "+Math.round(r.distanceToUser())+"m").limit(3).reduce((a,b)->a+", "+b).orElse("");HunterEvent event=HunterEvent.normal(h.configuredName+":routes:"+fresh.hashCode(),tr("event.tp_changed",h.name),now,Text.literal(summary)).typed(HunterEvent.Type.TELEPORT);if(h.source==HunterState.Source.AUTO_HIGH_OUTLAW&&h.threat.level().ordinal()<HunterState.ThreatLevel.HIGH.ordinal())notifications.activityOnly(event);else notifications.publish(event);}
    }
    private void updateNearbyCandidates(List<PlayerMarker> players,double px,double pz,double scale,long now,String localName){
        ...
        for(PlayerMarker p:players){
            ...
            if(distance<=config.hunterCandidateWarningRadius&&!state.inside){
                ...
                HunterEvent event=HunterEvent.warning("candidate:"+k+":nearby",Text.translatable("townymapaddon.hunter.warning.candidate_nearby",state.name,(int)Math.round(distance),state.outlawCount),now,
                        Text.translatable("townymapaddon.hunter.hud.name_distance",state.name,(int)Math.round(distance),bearing),
                        Text.translatable("townymapaddon.hunter.hud.claim_visible",state.claim),Text.translatable("townymapaddon.hunter.candidate.outlaw_count",state.outlawCount)).typed(HunterEvent.Type.CANDIDATE).positioned(p.x(),p.z());HunterState actor=candidateStates.get(k);if(actor!=null&&actor.threat.level().ordinal()>=HunterState.ThreatLevel.HIGH.ordinal())notifications.publish(event);
    ...
    private void recomputeTeleports(MinecraftClient mc,HunterState h,double px,double pz,String oldTown){
    ...
    private void updateHunter(HunterState h,PlayerMarker p,double px,double pz,long now){
        ...
        if(p!=null){
            ...
            if(old==HunterState.Visibility.VISIBLE&&h.source==HunterState.Source.MANUAL_HUNTER){double d=Math.hypot(h.direct.x()-px,h.direct.z()-pz);Text line=tr("event.last_seen",(int)Math.round(d),h.direct.claim());notifications.publish(d<=config.hunterNearbyRadius
        ...
        if(h.threat.level()!=oldThreat.level()){
            ...
            Text assessment=localizedAssessment(h.threat);Integer outlawCount=candidateOutlawCounts.get(key(h.name));Text source=h.source==HunterState.Source.AUTO_HIGH_OUTLAW?tr("source.auto_high_outlaw"):tr("source.manual");Text[] details=outlawCount==null?new Text[]{source,assessment}:new Text[]{source,assessment,Text.translatable("townymapaddon.hunter.candidate.outlaw_count",outlawCount)};
    ...
    private void publishFrontCrossings(HunterState h,List<net.townymap.hunter.front.HiddenThreatFrontEngine.Crossing> crossings,long now){for(var crossing:crossings){var origin=crossing.origin();boolean teleport=origin.type()==net.townymap.hunter.front.HiddenThreatOrigin.Type.TOWN_SPAWN||origin.type()==net.townymap.hunter.front.HiddenThreatOrigin.Type.NATION_SPAWN||origin.type()==net.townymap.hunter.front.HiddenThreatOrigin.Type.OTHER_TELEPORT;String front=crossing.type()==net.townymap.hunter.front.HiddenThreatFrontEngine.FrontType.WARNING?"warning":"plausible";Text title=tr("front."+(teleport?"teleport_":"")+front+"_crossed",h.name);Text detail=tr(teleport?"front.teleport_origin_reached":"front.origin_reached",origin.label(),(int)Math.round(crossing.playerDistance()));HunterEvent event=HunterEvent.warning(h.configuredName+":front:"+origin.key()+":"+crossing.type(),title,now,detail,teleport?tr(origin.type()==net.townymap.hunter.front.HiddenThreatOrigin.Type.NATION_SPAWN?"front.nation_spawn":"front.town_spawn"):origin.knownHunter()?tr("source.manual"):tr("source.auto_high_outlaw")).typed(teleport?HunterEvent.Type.TELEPORT:HunterEvent.Type.MOVEMENT).positioned((int)origin.x(),(int)origin.z());notifications.publish(event);}}
    ...
    public List<String> hunterHudLines(MinecraftClient mc){
        ...
        if(!rows.isEmpty()){out.add("§c§l"+Text.translatable("townymapaddon.hunter.watch.title").getString());for(int i=0;i<Math.min(config.hunterMaxHudEntries,rows.size());i++){Row r=rows.get(i);var o=r.h.bestObservation();String approx=r.h.visibility==HunterState.Visibility.VISIBLE?"":"~",bearing=config.hunterDirectionEnabled?direction(px,pz,o.x(),o.z()):"";out.add("§f"+Text.translatable("townymapaddon.hunter.hud.tracked",r.h.name,approx+(int)Math.round(r.d),bearing).getString());out.add(r.h.visibility==HunterState.Visibility.VISIBLE?"§7"+Text.translatable("townymapaddon.hunter.hud.visible_claim",o.claim()).getString():"§c"+Text.translatable("townymapaddon.hunter.hud.lost_claim",format(now-o.atMs()),o.claim()).getString());String detail=config.hunterShowRisk&&r.h.threat.level().ordinal()>=HunterState.ThreatLevel.WATCH.ordinal()?Text.translatable("townymapaddon.hunter.hud.risk_score",r.h.threat.level(),r.h.threat.score()).getString():"";var tp=r.h.threat.bestRoute();if(tp!=null)detail+=(detail.isBlank()?"":" · ")+Text.translatable("townymapaddon.hunter.hud.tp_threat",(int)Math.round(tp.distanceToUser()/s),tp.name()).getString();if(!detail.isBlank())out.add("§c"+detail);}if(rows.size()>config.hunterMaxHudEntries)out.add("§7"+Text.translatable("townymapaddon.hunter.hud.more",rows.size()-config.hunterMaxHudEntries).getString());}
        ...
        if(config.hunterCandidateWarningsEnabled){
            ...
            if(!automatic.isEmpty()&&available>0)out.add("§6§l⚠ "+Text.translatable("townymapaddon.hunter.hud.possible_hunter").getString());
            for(int i=0;i<Math.min(available,automatic.size());i++){NearbyCandidate c=automatic.get(i);String bearing=config.hunterDirectionEnabled?direction(px,pz,c.x,c.z):"";out.add("§f"+Text.translatable("townymapaddon.hunter.hud.name_distance",c.name,(c.visibleNow?"":"~")+(int)Math.round(c.distance),bearing).getString());out.add(c.visibleNow?"§7"+Text.translatable("townymapaddon.hunter.hud.claim_visible",c.claim).getString():"§c"+Text.translatable("townymapaddon.hunter.hud.position_lost",format(now-c.lastSeenAt)).getString());HunterState candidateThreat=candidateStates.get(key(c.name));String riskText=candidateThreat==null?"":" · "+risk(candidateThreat.threat.level()).getString();out.add("§7"+Text.translatable("townymapaddon.hunter.candidate.outlaw_count",c.outlawCount).getString()+riskText);}
        }
        return out;
    }
    public String safeRouteHudLine(MinecraftClient mc){if(!config.hunterWarningEnabled||!config.hunterShowHud||route==null||mc==null||mc.player==null)return "";double s=net.townymap.TownyMapMod.dimensionCoordinateScale(),px=mc.player.getX()*s,pz=mc.player.getZ()*s;int intersections=threatFronts.routeIntersectionCount(px,pz,route.x(),route.z(),System.currentTimeMillis());return intersections>0?"§c"+tr("hud.route_front_risk",intersections,route.destination()).getString():"§a"+tr("hud.safe_route",config.hunterDirectionEnabled?direction(px,pz,route.x(),route.z()):"",(int)Math.round(route.distance()/s),route.destination()).getString();}
    ...
    private boolean infer(HunterState h,String other,long now){
        ...
        Text timing=tr("event.victim_timing",format(age),confidence(c));
    ...
    private static int sx(double x,double camera,double scale,int size){return(int)Math.round((x-camera)*scale+size/2.0);}private static int sy(double z,double camera,double scale,int size){return(int)Math.round((z-camera)*scale+size/2.0);}private static void cross(DrawContext c,int x,int y,int color){c.fill(x-5,y-1,x+6,y+2,color);c.fill(x-1,y-5,x+2,y+6,color);}private static void dottedLine(DrawContext c,int x1,int y1,int x2,int y2,int color){int steps=Math.min(300,Math.max(Math.abs(x2-x1),Math.abs(y2-y1)));if(steps<=0)return;for(int i=0;i<=steps;i+=6){int x=x1+(x2-x1)*i/steps,y=y1+(y2-y1)*i/steps;c.fill(x-1,y-1,x+2,y+2,color);}}
    private static Text tr(String suffix,Object...args){return Text.translatable("townymapaddon.hunter."+suffix,args);}
    private static Text townOrNone(String town){return town==null||town.isBlank()?tr("common.no_town"):Text.literal(town);}
    private static Text risk(HunterState.ThreatLevel level){return tr("risk."+level.name().toLowerCase(Locale.ROOT));}
    private static Text confidence(HunterState.Confidence value){return tr("confidence."+value.name().toLowerCase(Locale.ROOT));}
    private static Text localizedAssessment(HunterState.ThreatAssessment a){var route=a.bestRoute();Text components=tr("assessment.components",risk(a.physicalLevel()),risk(a.teleportLevel()));if(route==null)return components;return tr("assessment.route",components,route.name(),(int)Math.round(route.distanceToUser()),tr("phase."+a.hiddenPhase().replace('-','_')));}
