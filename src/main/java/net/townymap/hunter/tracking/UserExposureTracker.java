...
public final class UserExposureTracker {
    ...
    public double targetingMultiplier(long now,boolean enabled,long delayMs,long rampMs,double maximumReduction){if(!enabled||visible)return 1;long beyond=Math.max(0,stateDurationMs(now)-Math.max(0,delayMs));double progress=Math.clamp((double)beyond/Math.max(1,rampMs),0,1);return Math.clamp(1-progress*Math.clamp(maximumReduction,0,.8),.2,1);}
