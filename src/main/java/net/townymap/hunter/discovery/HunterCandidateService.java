...
public final class HunterCandidateService {
    ...
    public void refresh(List<TownData> towns){if(towns==null||towns.isEmpty()||!refreshing.compareAndSet(false,true))return;List<String>names=towns.stream().map(TownData::name).toList();api.fetchOutlawTownIndex(names).whenComplete((next,error)->{Minecraft mc=Minecraft.getInstance();if(mc==null){refreshing.set(false);return;}mc.execute(()->{try{if(error==null&&next!=null&&next.townsScanned()>0){snapshot=next;failed=false;}else{failed=true;TownyMapMod.LOGGER.warn("[TownyMap] Hunter candidate outlaw-index refresh failed; retaining cached results");}}finally{refreshing.set(false);}});});}
